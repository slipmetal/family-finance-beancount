...
#:
#: `account` — счёт, ПО которому идёт выписка. Нужен, когда одно и то же
#: описание значит разное на разных счетах: «Пополнение. Сбербанк Онлайн» на
#: одном счёте — перевод со своего же счёта, а на другом — деньги извне.
#: Не путать с ключом `account` верхнего уровня: тот говорит, КУДА пойдёт
#: вторая нога, а этот — на каких выписках правило вообще применимо.
TEXT_FIELDS = ("counterparty", "details", "type", "correspondent", "account", "text")
    ...
    def apply(
        ...
        *,
        ...
        fields = {
            ...
            # Счёт, по которому идёт выписка. В `text` тоже НЕ входит: иначе
            # правило на мерчанта могло бы зацепиться за имя счёта.
            "account": account,
        }
